...
function NewReview() {
  ...
  const { imdbID } = useParams<RouteParams>(); 
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        "https://ms-2-project-backend.onrender.com/reviews",
        {
          method: "POST",
          headers: {
          ...
          <Paper elevation={10} style={paperStyle}>
            <Grid container justifyContent="center" alignItems="center" spacing={2}>
              <Grid item xs={12} alignItems="center">
                ...
                {ratingError && (
                  <Typography color="error">Please provide at least a 1-star rating.</Typography>
                )}
              </Grid>
              <Grid item xs={12}>
                <form
                ...
                >
                  ...
                  />
                  <Typography>
                    {review.length}/{maxCharacters} characters
                  </Typography>
                  <Box sx={{ display: "flex", justifyContent: "space-between", mt: 2 }}>
                    <Button
                      type="button"
                      onClick={handleCancel}
                      sx={{ backgroundColor: "red", color: "white", px: 3 }}
                      variant="contained"
                    >
                      Cancel
                    </Button>
                    <Button
                      type="submit"
                      sx={{ backgroundColor: "blue", color: "white", px: 3 }}
                      variant="contained"
                    >
                      Submit
                    </Button>
