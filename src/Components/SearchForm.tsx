...
const SearchForm: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<SearchResults | null>(null); // State to hold search results
    ...
    <div>
      <Box display="flex" justifyContent="center" mb={2}>
        <TextField
          ...
          onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === 'Enter') fetchApiData(query); }}
          InputProps={{
            startAdornment: (
      ...
      {searchResults && searchResults.Search && (
        <Box display="flex" justifyContent="center">
          <Grid container spacing={2} style={{ margin: '30px auto', maxWidth: '1400px' }}>
            {searchResults.Search.map((movie: Movie) => (
